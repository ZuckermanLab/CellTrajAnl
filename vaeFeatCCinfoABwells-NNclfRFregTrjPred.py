...
with open(f"performanceNN_{fovs[test_fov]}_trajl{trajl}_VAE{latent_dim}Cls{num_classes}_.json", 'w') as fout:
...
performance_data_RF = {
    "Mean Squared Error": f"{mse_testRF: .3f}%",
    "R2": f"{R2_testRF: .3f}%",
    "Pearsons R": f"{R_testRF: .3f}%"
}
with open(f"performanceRF-NN_{fovs[test_fov]}_trajl{trajl}_VAE{latent_dim}Cls{num_classes}.json", 'w') as fout:
    json.dump(performance_data_RF, fout, indent=4)

# Get Indices of the Full Trajectory & its link to "Test Set Data"
cc_filename = f"{file_path}{sname_reporter}_{fovs[test_fov]}.dat" 
dummy, dummy, f_nums = get_cellCycInfoTrajs(cc_filename) # Get Data along all single-cell trajectories

frame_nums_test = []
full_trj_inds = []

indstm = np.where(indtreatment_traj == test_fov)[0]
fid_snippets = indframes_traj[indstm].astype(int) # Map frame indices of snippets
indc_map_fulltraj_snippets = indmodel_traj_snippets[indstm].astype(int) # Map Indices of Full Trajectory to Snippets 

for j, ind_fulltraj in enumerate(indc_map_fulltraj_snippets):
    possible_indices = np.where(f_nums[ind_fulltraj] == fid_snippets[j])[0] # Look for a snippet frame within its full trajectory
    if len(possible_indices) > 0:
        frame_num = f_nums[ind_fulltraj][possible_indices[0]]
        frame_nums_test.append(frame_num)
        full_trj_inds.append(ind_fulltraj)
frame_nums_test = np.array(frame_nums_test, dtype=int)
full_trj_inds = np.array(full_trj_inds, dtype=int)

trajl_cutoff = 150 # See longer trajectories 

fig, axs = plt.subplots()

def animate(traj_index):
    indices = np.where(full_trj_inds == traj_index)
    if indices[0].size > trajl_cutoff:
        axs.clear()
        axs.set_title(f'CNN+DNN+LASSO: {fovs[test_fov]}')
        axs.plot(frame_nums_test[indices], y_test[indices], color='k', label='Actual')
        axs.plot(frame_nums_test[indices], y_pred_lasso[indices], color='r', label='Predicted')
        axs.legend(loc='best')
        plt.tight_layout()

ani = animation.FuncAnimation(fig, animate, frames=range(full_trj_inds.size), interval=10)
# Save Animation in a File 
ani.save(f"animRF-NN_{fovs[test_fov]}_trajl{trajl}_VAE{latent_dim}Cls{num_classes}.gif", writer='pillow')