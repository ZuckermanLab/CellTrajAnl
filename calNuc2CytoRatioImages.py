...
sysName = 'LI204601_G'
h5pathSet = '/home/groups/ZuckermanLab/jalim/LI204601_INCUCYTE/ratioNucCyto/'

# Function to Scale Images in the range of (0, 1)
...
def nucByCytoRatioSingleCell(cropped_reporter_image, nuclei_within_cell_mask, cell_mask):
    ...
    # Sum the intensities within the nucleus & the Cyto region
    sum_intensity_nuc = np.nansum(nucleus_region, dtype = np.float64)
    sum_intensity_cyto = np.nansum(cyto_region, dtype = np.float64)
    #print(f"Sum Intensity Nucleus: {sum_intensity_nuc}, Sum Intensity Cytoplasm: {sum_intensity_cyto}")
    # Make sure there is no division by zero and the sums are not NaN
    if sum_intensity_cyto != 0. and sum_intensity_nuc != 0.:
        nuc_by_cyto = sum_intensity_nuc / sum_intensity_cyto
    else:
        nuc_by_cyto = np.nan 
    #print(f"Nuc/Cyto Ratio: {nuc_by_cyto}")
...
def scale_image(image):
    ...
    if np.ma.is_masked(image):
        ...
        img_scaled = scaler.fit_transform(data) # Fit the scaler to the image data and transform it
        img_scaled = img_scaled.reshape(data.shape) # Reshape the scaled image back to its original shape
        return np.ma.masked_array(img_scaled, mask = mask)
    else:
...
for im in indgood_models:
    ...
    fileCC_dump = sysName+'_'+conditions[inds_tmSet_models[im]]+'_'+str(fovs)+'.dat'
    ...
    for sctraj_indc in range(num_single_cell_trajs):
        ...
        for itt in range(len_single_cell_traj):
            ...
            # Scale the area of interest within the images to (0, 1)
            cropped_reporter_image = scale_image(cropped_reporter_image)
